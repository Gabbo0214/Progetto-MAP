    ...
    public void talkTo(Player p, Stobj person) {
        if (person.getName().equals("Custode")) {
            ...
            if (!hasInitialDialogOccurred) {
                this.setMsg("Il custode ti osserva attentamente e dice: 'Benvenuto, avventuriero. Sono il custode dei segreti antichi di questo luogo. Sei giunto qui in cerca di risposte, e io posso aiutarti. Ascolta attentamente, perché ciò che ti sto per dire sarà fondamentale per il tuo cammino. So che stai provando a recuperare il famoso “anello dell’eternità”, non sarà per niente facile. Per poter affrontare l'enigma avrai bisogno di molta saggezza e conoscenza. Ti darò un indizio che potrebbe rivelarsi cruciale per la tua ricerca: 'Nel silenzio dei segreti, troverai la verità nascosta.' Che la saggezza del passato ti guidi verso la tua destinazione.'\n");
            ...
            if (!this.open) {
                if (a) {
                    ...
                    if (c)
                        this.setMsg(this.getMsg() + "Notando che porti con te solo una misera spada, ti avvisa che potrebbe servire un'arma migliore...");
                    ...
                    this.setDescription("Ti avvicini a nord della Sala della luna. Dato che hai con te un'arma per difenderti, il custode ti ha concesso di passare aprendoti la porta.");
                } else {
                    this.setMsg(this.getMsg() + "Il custode ti blocca e ti dice che hai bisogno di un'arma per difenderti. Ti consiglia di andare nell'unica armeria del castello o almeno cercare qualcosa da usare nell'entrata del castello!");
                }
            } else if (a) {
